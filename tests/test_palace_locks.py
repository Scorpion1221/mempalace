...
def test_lock_holder_identity_persists_across_release(tmp_path, monkeypatch):
    ...
    # it by scanning the mempalace locks dir for the canonical palace-write
    # lock file. (The canonical helper is mine_palace_lock; the on-disk name
    # is retained for the writer-lock alias.)
    lock_dir = tmp_path / ".mempalace" / "locks"
    lock_files = list(lock_dir.glob("palace_write_*.lock"))
