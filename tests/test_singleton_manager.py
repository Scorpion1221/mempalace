...
def short_sockdir(tmp_path):
    ...
    probe = base / "probe.sock"
    probe_server = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
    try:
        probe_server.bind(str(probe))
    except PermissionError:
        pytest.skip("AF_UNIX bind is not permitted in this sandbox")
    finally:
        probe_server.close()
        if probe.exists():
            probe.unlink()
