...
_HASH_EF_CACHE: object = "UNSET"
_WARNED: set = set()

_HASH_EMBEDDING_MODELS = {"hash", "deterministic", "test"}


class HashEmbeddingFunction(chromadb.EmbeddingFunction):
    """Small fully-local deterministic embedding function.

    This is intentionally opt-in via ``MEMPAL_EMBEDDING_MODEL=hash`` (or the
    synonyms in ``_HASH_EMBEDDING_MODELS``). It gives tests and constrained
    development environments a no-network embedding path while preserving the
    persisted Chroma embedding-function identity (``"default"``) expected by
    palaces created with Chroma's default MiniLM function.
    """
    ...
    @staticmethod
    def build_from_config(config) -> "HashEmbeddingFunction":
        HashEmbeddingFunction.validate_config(config)
        dims = config.get("dimensions", 384) if isinstance(config, dict) else 384
        return HashEmbeddingFunction(dimensions=int(dims))
...
def _proxy_model_from_env() -> str:
    ...
    if not model or model.lower() == "default":
        return ""
    if model.lower() in _HASH_EMBEDDING_MODELS:
        return ""
    return model


def _hash_model_enabled() -> bool:
    model = os.environ.get("MEMPAL_EMBEDDING_MODEL", "").strip().lower()
    return model in _HASH_EMBEDDING_MODELS


def _get_hash_embedding_function():
    global _HASH_EF_CACHE
    if not _hash_model_enabled():
        return None
    if _HASH_EF_CACHE != "UNSET":
        return _HASH_EF_CACHE
    dims_str = os.environ.get("MEMPAL_EMBEDDING_DIMS", "")
    dims = int(dims_str) if dims_str.strip().isdigit() else 384
    _HASH_EF_CACHE = HashEmbeddingFunction(dimensions=dims)
    return _HASH_EF_CACHE
...
def get_embedding_function(device: Optional[str] = None):
    ...
    Policy:
    * If ``MEMPAL_EMBEDDING_MODEL=hash`` (or deterministic/test), use the
      fully-local deterministic hash embedding function.
    * If ``MEMPAL_EMBEDDING_MODEL`` is set (and not ``default``/``hash``),
      use Sir's OpenAI-compatible proxy embedding function.
    * Otherwise use upstream's local ONNX MiniLM embedding function with the
    ...
    """
    hash_ef = _get_hash_embedding_function()
    if hash_ef is not None:
        return hash_ef
...
def reset_cache():
    ...
    global _EF_CACHE, _PROXY_EF_CACHE, _HASH_EF_CACHE
    _EF_CACHE = {}
    _PROXY_EF_CACHE = "UNSET"
    _HASH_EF_CACHE = "UNSET"
