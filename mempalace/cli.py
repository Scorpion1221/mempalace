    ...
    if not args.dry_run:
        try:
            comp_col = backend.get_or_create_collection(
                palace_path, "mempalace_closets", embedding_function=ef
            )
...
def main():
    ...
    p_repair = sub.add_parser(
        "repair",
        help=(
            "Rebuild palace vector index from stored data, rebuild from verbatim, "
            "or un-poison max_seq_id rows"
        ),
    )
    ...
    p_singleton = sub.add_parser(
        "singleton",
        help=(
            "Manage the shared local MemPalace MCP singleton "
