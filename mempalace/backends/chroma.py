...
class ChromaBackend(BaseBackend):
    ...
    * An inode+mtime freshness check absorbed from ``mcp_server._get_client``
    ...
    @staticmethod
    def _db_stat_full(palace_path: str) -> tuple[int, int, int] | None:
        """Return ``(size, mtime_ns, inode)`` for write-path freshness.

        ``None`` means the DB file is absent. Keeping the absent state distinct
        from a real all-zero tuple lets write-path cache invalidation detect
        delete/recreate transitions without inventing fake filesystem stats.
        """
        db_path = os.path.join(palace_path, "chroma.sqlite3")
        try:
            st = os.stat(db_path)
    ...
    def get_collection(
        ...
        * Legacy: ``get_collection(palace_path, collection_name, create=False)``
        ...
        ef = (
            ...
            else self._resolve_embedding_function()
        )
