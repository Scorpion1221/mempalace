...
This module provides five operations:
  ...
  rebuild               — extract all drawers, stage in a temp collection, then swap live
  rebuild_from_verbatim — rebuild from verbatim documents in chroma.sqlite3
...
class RebuildFromVerbatimReport(RebuildReport):
    """Backward-compatible name for the rebuild-from-verbatim report."""
...
def rebuild_from_verbatim(
    ...
    *,
    ...
    if not palace.exists():
        raise FileNotFoundError(f"palace path does not exist: {palace}")
    if not (palace / "chroma.sqlite3").is_file():
        raise ValueError(f"no chroma.sqlite3 in {palace}; nothing to rebuild from")

    from .palace import palace_write_lock

    with palace_write_lock(str(palace), timeout=_REBUILD_LOCK_TIMEOUT_S):
        ...
        if not counts:
            raise ValueError(f"rebuild_from_sqlite refused to rebuild {palace}")
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "command", choices=["status", "scan", "prune", "rebuild", "rebuild-from-verbatim"]
    )
    ...
    if args.command == "status":
        ...
        def _progress(phase: str, processed: int, total: int) -> None:
            if phase == "done":
                print(f"  Done: {processed}/{total} rows rebuilt.")

        report = rebuild_from_verbatim(target, progress_cb=_progress)
