...
def _get_collection(create=False):
    ...
    Merges the fork's write-refresh contract (the returned wrapper carries
    backend/path/collection/EF metadata so refresh_for_write() can rebind under
    palace_write_lock) with upstream v3.3.5's stale-handle retry and HNSW
    runtime pinning. On failure we clear caches and retry once.
    """
    global \
        _client_cache, \
        _collection_cache, \
...
def tool_add_drawer(
    ...
    try:
        with palace_write_lock(_config.palace_path, timeout=_PALACE_WRITE_LOCK_TIMEOUT_S):
            refresh_for_write = getattr(col, "refresh_for_write", None)
            if callable(refresh_for_write):
                refresh_for_write()
...
def main():
    ...
    # Pre-flight: probe HNSW capacity before any tool call so the warning is
    # visible at startup rather than on first use (#1222). Pure filesystem
    # read; never opens a chromadb client.
    _refresh_vector_disabled_flag()
